        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          {FEATURED.map((f, i) => (
            <article key={f.name} className="reveal lift bg-cream rounded-xl overflow-hidden border border-ink/8 flex flex-col" style={{ transitionDelay: `${i * 80}ms` }}>
              <div className="overflow-hidden"><ProductMockup src={f.src} label={f.name} aspect="4/3" bg="#FFFFFF" /></div>
              <div className="p-7 lg:p-8 flex flex-col flex-1">
                ...
                <div className="mt-6 pt-5 border-t border-ink/12 flex items-baseline justify-between">
                  ...
                  <Link href="#" className="text-leaf font-medium text-[14px] inline-flex items-center gap-1.5">Ver caso <Icon.arrow className="w-4 h-4" /></Link>
                </div>
              </div>
            </article>
          ))}
    ...
    <section className="bg-cream">
      <div className="mx-auto max-w-site px-6 lg:px-10 py-24 lg:py-32">
        <div className="mb-14 reveal max-w-3xl">
          ...
          <h2 className="font-serif text-ink" style={{ fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 0.98, fontSize: "clamp(2.2rem, 5vw, 4rem)" }}>
            Empacamos seis industrias.<br />
            <em style={{ fontWeight: 500, color: "#E11D2A" }}>Todas, comida.</em>
          </h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <ul className="lg:col-span-7 space-y-px bg-ink/10 rounded-xl overflow-hidden">
            {INDUSTRIES.map((ind, i) => {
              const IconC = Icon[ind.icon as keyof typeof Icon];
          ...
          <div className="lg:col-span-5 reveal">
            <div className="bg-ink text-paper rounded-xl p-8 lg:p-10 sticky top-32">
              ...
              <Link href="/contacto" className="mt-8 btn btn-primary !w-full justify-center">Cotiza para tu industria <Icon.arrow className="w-4 h-4" /></Link>
            </div>
          </div>
        </div>
      </div>
