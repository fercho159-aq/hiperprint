...
const TIMELINE = [
  ...
  { y: "2023", t: "1,500 clientes activos",     b: "Producimos para hamburgueserías, taquerías, heladerías, panaderías y dulcerías en 28 estados." },
  { y: "2026", t: "Hoy",                        b: "Tres turnos de producción, 32 personas en equipo, 200,000 piezas mensuales en línea continua." },
];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          <div className="lg:col-span-7 reveal">
            ...
            <h2 className="mt-4 font-serif text-ink" style={{ fontWeight: 700, letterSpacing: "-0.03em", lineHeight: .98, fontSize: "clamp(2.4rem, 5.5vw, 4.8rem)" }}>
              Imprimimos los <em style={{ fontWeight: 500, color: "#C84B31" }}>antojos</em> de México desde 2009.
            </h2>
          ...
          <div className="lg:col-span-5 reveal">
            <ProductMockup label="planta-iztapalapa-fachada.jpg" tone="kraft" aspect="4/5" note="PLANTA · CDMX" />
      ...
      <div className="mx-auto max-w-site px-6 lg:px-10 py-24 lg:py-32">
        <div className="mb-16 reveal max-w-3xl">
          ...
          <h2 className="font-serif text-ink" style={{ fontWeight: 700, letterSpacing: "-0.03em", lineHeight: .98, fontSize: "clamp(2.2rem, 5vw, 4rem)" }}>
            De una máquina<br/><em style={{ fontWeight: 500, color: "#3D5B3A" }}>a una planta entera</em>.
          </h2>
        </div>
        <div className="relative">
          ...
          <ol className="space-y-12 lg:space-y-16">
            {TIMELINE.map((m, i) => {
              const right = i % 2 === 0;
              return (
                <li key={m.y} className="reveal relative md:grid md:grid-cols-2 md:gap-12" style={{ transitionDelay: `${i*60}ms` }}>
                  {/* dot */}
                  <span className="absolute left-[10px] md:left-1/2 top-2 -translate-x-1/2 w-3 h-3 rounded-full bg-leaf ring-4 ring-cream" />
                  <div className={`pl-12 md:pl-0 ${right ? "md:text-right md:pr-10" : "md:col-start-2 md:pl-10"}`}>
                    <div className="stat-num text-tortilla" style={{ fontSize: "clamp(2.4rem, 4vw, 3.5rem)" }}>{m.y}</div>
                    <h3 className="mt-2 font-serif text-ink" style={{ fontWeight: 600, fontSize: "clamp(1.4rem, 2.3vw, 1.9rem)", letterSpacing: "-0.02em", lineHeight: 1.15 }}>{m.t}</h3>
                    <p className="mt-3 text-ink/70 text-[15.5px]" style={{ lineHeight: 1.55 }}>{m.b}</p>
                  </div>
                </li>
              );
            })}
    ...
    <section className="bg-paper">
      <div className="mx-auto max-w-site px-6 lg:px-10 py-24 lg:py-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-ink/10">
          {VALUES.map((v, i) => (
            <div key={v.n} className="bg-paper p-8 lg:p-12 reveal" style={{ transitionDelay: `${i*80}ms` }}>
              <div className="flex items-center justify-between mb-6">
                <span className="stat-num text-ink/15" style={{ fontSize: "clamp(2.5rem, 4vw, 3.5rem)" }}>{v.n}</span>
                <span className="font-mono text-[11px] tracking-[.16em] uppercase text-ink/45">REGLA · {v.n}</span>
              </div>
              <h3 className="font-serif text-ink" style={{ fontWeight: 600, fontSize: "clamp(1.6rem, 2.8vw, 2.2rem)", letterSpacing: "-0.02em", lineHeight: 1.1 }}>{v.t}</h3>
              <p className="mt-4 text-ink/70 text-[16px]" style={{ lineHeight: 1.6 }}>{v.b}</p>
            </div>
          ))}
        </div>
      </div>
...
function Planta() {
  return (
    <section id="planta" className="grain relative" style={{ background: "#D4B896" }}>
      <div className="mx-auto max-w-site px-6 lg:px-10 py-24 lg:py-32">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          <div className="lg:col-span-5 reveal">
            ...
            <div className="mt-10 grid grid-cols-2 gap-x-6 gap-y-6">
              {TEAM_STATS.map((s) => (
                <div key={s.k}>
                  <div className="stat-num text-ink" style={{ fontSize: "clamp(2.2rem, 4vw, 3rem)" }}>{s.k}</div>
                  <div className="font-serif text-ink mt-1" style={{ fontWeight: 600, fontSize: "16px", letterSpacing: "-0.01em" }}>{s.v}</div>
                  <div className="font-mono text-[11px] tracking-wider uppercase text-ink/55 mt-1">{s.s}</div>
                </div>
              ))}
            </div>
          </div>
          <div className="lg:col-span-7 reveal">
            <div className="grid grid-cols-6 gap-3">
              <div className="col-span-4 row-span-2"><ProductMockup label="planta-area-produccion.jpg" tone="ink" aspect="4/5" note="PRODUCCIÓN" /></div>
              <div className="col-span-2"><ProductMockup label="planta-troqueladora.jpg" tone="deep" aspect="1/1" note="TROQUEL" /></div>
              <div className="col-span-2"><ProductMockup label="planta-equipo-diseno.jpg" tone="cream" aspect="1/1" note="DISEÑO" /></div>
              <div className="col-span-3"><ProductMockup label="planta-rollos-papel.jpg" tone="kraft" aspect="3/2" note="MATERIA PRIMA" /></div>
              <div className="col-span-3"><ProductMockup label="planta-bodega-envio.jpg" tone="sage" aspect="3/2" note="LOGÍSTICA" /></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="eco" className="bg-leaf text-paper">
      <div className="mx-auto max-w-site px-6 lg:px-10 py-24 lg:py-32">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-7 reveal">
            <span className="pill-eco !bg-paper/15 !text-paper"><Icon.leaf className="w-3.5 h-3.5" /> Sustentabilidad</span>
            <h2 className="mt-6 font-serif" style={{ fontWeight: 700, letterSpacing: "-0.03em", lineHeight: .96, fontSize: "clamp(2.4rem, 5.5vw, 4.5rem)" }}>
              No es marketing.<br/><em style={{ fontWeight: 500, color: "#D4B896" }}>Es operación</em>.
            </h2>
            <div className="mt-8 space-y-5 text-paper/85 text-[16.5px] lg:text-[17px]" style={{ lineHeight: 1.65 }}>
              <p>
            ...
            <a href="#" className="mt-8 inline-flex items-center gap-2 font-medium text-paper hover:gap-3 transition-all">
              Descargar reporte 2025 <Icon.arrow className="w-4 h-4" />
            </a>
          </div>
          <div className="lg:col-span-5 reveal">
              ...
              {CERTS.map((c) => (
                <div key={c.k} className="bg-leaf p-6 flex items-start gap-5">
                  <div className="shrink-0 w-16 text-center">
                    <div className="font-serif text-paper" style={{ fontWeight: 700, fontSize: 22, letterSpacing: "-0.02em" }}>{c.k}</div>
                  </div>
                  <div>
                    <div className="font-serif text-paper text-[18px]" style={{ fontWeight: 600 }}>{c.l}</div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 reveal order-2 lg:order-1">
            ...
            <div className="mt-8 flex items-center gap-4">
              ...
              <div>
                <div className="font-serif text-ink text-[18px]" style={{ fontWeight: 600 }}>Alejandro Hernández</div>
                <div className="text-ink/60 text-[14px] font-mono tracking-wide">Fundador · Hiperprint</div>
              </div>
            </div>
          </div>
          <div className="lg:col-span-7 reveal order-1 lg:order-2">
            <ProductMockup label="fundador-piso-planta.jpg" tone="deep" aspect="4/3" note="EQUIPO · 2026" />
          </div>
        </div>
      </div>
    </section>
